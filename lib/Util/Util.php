...
abstract class Util
{
    ...
    public static function convertToStripeObject($resp, $opts)
    {
        ...
        if (self::isList($resp)) {
            ...
            if (isset($resp['object']) && is_string($resp['object']) && isset($types[$resp['object']])) {
                $class_reflection = $types[$resp['object']];
            } else {
                $class_reflection = new \ReflectionClass('\Stripe\StripeObject');
            }
            $method = $class_reflection->getMethod('constructFrom');
            return $method->invoke(null, $resp, $opts);
        } else {
            return $resp;
        }
    }
    ...
    public static function getTypes(): array {
        if (!empty(self::$type_map)) {
            return self::$type_map;
        }
        $types = [];
        foreach (\get_declared_classes() as $classname) {
            if (0 !== \strpos($classname, 'Stripe')) {
                continue;
            }

            $class_reflection = new \ReflectionClass($classname);
            $constants = $class_reflection->getConstants();
            if (!\array_key_exists('OBJECT_NAME', $constants)) {
                continue;
            }
            $types[(string) $constants['OBJECT_NAME']] = $class_reflection;
        }
        self::$type_map = $types;
        return self::$type_map;
    }
